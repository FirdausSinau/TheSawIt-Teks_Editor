...
Text Editor CLI v2 adalah aplikasi text editor berbasis terminal (CLI) yang dibuat menggunakan bahasa C. Proyek ini dikembangkan untuk menerapkan konsep struktur data **Singly Linked List**, di mana setiap baris teks disimpan dalam satu node yang terhubung secara berantai.
...
**Catatan:**
- Tanda `[*]` pada nama file menandakan terdapat perubahan yang belum disimpan.
- Tanda `>` di samping nomor baris menandakan baris yang sedang aktif.

---

## Cara Kompilasi dan Menjalankan

Pastikan compiler C (misalnya `gcc`) telah terinstal pada sistem.

**Kompilasi** (jalankan di terminal atau command prompt):

```bash
gcc main.c Anand.c Firdaus.c Faiq.c file.c display.c replace.c -o editor
```
...
Atau langsung membuka file tertentu saat program dimulai:
